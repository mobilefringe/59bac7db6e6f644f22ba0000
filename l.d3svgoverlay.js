...
L.D3SvgOverlay = (L.version < "1.0" ? L.Class : L.Layer).extend({
    ...
    _zoomChange: function (evt) {
        ...
        this._disableLeafletRounding();
        var newZoom = this._undef(evt.zoom) ? this.map._zoom : evt.target._zoom; // "viewreset" event in Leaflet has not zoom/center parameters like zoomanim
    ...
    onAdd: function (map) {
        ...
        if (L.version < "1.0") {
            ...
            $('.'+ overlayClass +' svg').children().each (function (key,val){
                $("#"+val.id).addClass("leaflet-zoom-animated");
                temp_g.push(val);
            });
